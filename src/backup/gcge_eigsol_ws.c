		...
		GCGE_OPS *ops, void *A)
{
    ...
    ops->MultiVecCreateByMat(&((*eigsol_ws)->V),       size_V,       A, ops);
    ops->MultiVecCreateByMat(&((*eigsol_ws)->V_tmp),   size_V_tmp,   A, ops);
    ops->MultiVecCreateByMat(&((*eigsol_ws)->RitzVec), size_RitzVec, A, ops);
}
